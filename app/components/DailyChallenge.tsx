...
export function DailyChallenge({ kind, target, countries, language, difficulty, onResolved, onContinue }: { kind: Extract<ModeId, "daily-capital" | "capital-wordle" | "flag-choice" | "geo-connection">; target: Country; countries: Country[]; language: Language; difficulty: Difficulty; onResolved: (correct: boolean) => void; onContinue: () => void }) {
  ...
  const countryOptions = useMemo(() => countries.map((country) => ({ label: getCountryDisplayName(country, language), searchTerms: country.acceptedNames })), [countries, language]);
  const capitalOptions = useMemo(() => countries.map((country) => ({ label: getCapitalDisplayName(country, language), searchTerms: country.acceptedCapitals })), [countries, language]);
  ...
  function submit(event: FormEvent) {
    ...
    if (kind === "capital-wordle") {
      ...
      if (value.length !== capitalAnswer.length) return;
      if (difficulty === "hard" && !capitalWords.has(value)) return;
  ...
  if (kind === "geo-connection") return <section className="daily-game-shell"><h1>{language === "es" ? "País oculto" : "Hidden country"}</h1><div className="connection-clues">{connectionClues.slice(0, clues).map((clue) => <div key={clue}>{clue}</div>)}</div>{clues < connectionClues.length && <button className="clue-button" onClick={() => setClues((value) => value + 1)}>{copy.clue}</button>}<form className="daily-answer-form" onSubmit={submit}><AnswerAutocomplete value={answer} onChange={setAnswer} options={countryOptions} placeholder={copy.country} autoFocus /><button>{copy.check}</button></form></section>;

  return <section className="daily-game-shell"><h1>{kind === "daily-capital" ? countryName : (language === "es" ? "Capital Wordle" : "Capital Wordle")}</h1>{kind === "daily-capital" && difficulty === "easy" && <p className="daily-prompt">{getGeographyName(target.region, language)} · {getGeographyName(target.subregion, language)}</p>}{kind === "capital-wordle" && <><p className="daily-prompt">{guesses.length}/{maxTries} {copy.tries} · {capitalAnswer.length} {language === "es" ? "letras" : "letters"}</p><div className="capital-wordle-board">{guesses.map((guess, row) => <div className="capital-wordle-row" key={row}>{Array.from({ length: capitalAnswer.length }, (_, index) => <span className={guess.status[index] || ""} key={index}>{guess.value[index] || ""}</span>)}</div>)}</div></>}<form className="daily-answer-form" onSubmit={submit}>{kind === "capital-wordle" ? <input value={answer} onChange={(event) => setAnswer(event.target.value)} maxLength={capitalAnswer.length} placeholder={copy.capital} autoFocus /> : <AnswerAutocomplete value={answer} onChange={setAnswer} options={capitalOptions} placeholder={copy.capital} autoFocus />}<button>{copy.check}</button></form></section>;
}
