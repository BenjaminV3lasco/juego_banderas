...
export function NeighbourCountries({ target, countries, language, difficulty, onResolved, onContinue }: { target: Country; countries: Country[]; language: Language; difficulty: Difficulty; onResolved: (correct: boolean) => void; onContinue: () => void }) {
  ...
  if (finished !== null) return <section className="neighbour-shell neighbour-finish"><span className={finished ? "win" : "lose"}>{finished ? copy.win : copy.lose}</span><h1>{copy.answer}: {targetName}</h1><div className="standard-flag-frame"><Image src={target.flag} alt={targetName} width={320} height={180} unoptimized /></div><button onClick={onContinue}>{copy.next}</button></section>;
